#!/usr/bin/env node
/**
 * TiendaMiBarrio - Aplica la migración 015 (expenses.pos_id, purchases.pos_id) de forma segura.
 *
 * Uso: node scripts/apply-migration-015.js
 *
 * Lee las credenciales de .env.local (o variables de entorno ya cargadas) y
 * ejecuta las sentencias de mysql/migration-015-pos-gastos-compras.sql.
...
async function main() {
  ...
  // Leer las sentencias de la migración 015 (fuente única de verdad).
  const migrationPath = path.join(__dirname, '../mysql/migration-015-pos-gastos-compras.sql');
  ...
  console.log(`✅  Migración 015 aplicada (${ok} sentencias, ${skipped} ya existían).`);
  await conn.end();
  console.log('\n🎉  Gastos y compras vinculados a caja.');
}
