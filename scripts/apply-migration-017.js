/**
 * Aplica la migración 017: columna invoice_number en purchases.
 *
 * Uso: node scripts/apply-migration-017.js
 *
 * Lee las credenciales de .env.local (o variables de entorno ya cargadas) y
 * ejecuta las sentencias de mysql/migration-017-purchases-invoice.sql.
...
async function main() {
  ...
  // Leer las sentencias de la migración 017 (fuente única de verdad).
  const migrationPath = path.join(__dirname, '../mysql/migration-017-purchases-invoice.sql');
  ...
  console.log(`✅  Migración 017 aplicada (${ok} sentencias, ${skipped} ya existían).`);
  await conn.end();
  console.log('\n🎉  Campo invoice_number disponible en purchases.');
}
