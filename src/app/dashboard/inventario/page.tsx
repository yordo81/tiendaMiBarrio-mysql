...
import { Package, Plus, Search, Edit2, Trash2, AlertTriangle, Tag, History, ArrowRightLeft } from 'lucide-react';
...
export default function InventarioPage() {
  ...
  const [moveForm, setMoveForm] = useState({ type: 'in', quantity: 0, reason: '' });
  ...
  const load = useCallback(async () => {
    ...
    setProducts(prods); setCategories(cats); setSuppliers(sups); setLoading(false);
  }, []);

  useEffect(() => { load(); }, [load]);

  function openNew() { setEditProduct(null); setForm({ name:'', sale_price:0, cost:0, stock:0, min_stock:0, unit:'unidad', supplier_ids:[] }); setShowModal(true); }
  ...
  async function handleMove() {
    ...
    setSaving(true);
    try {
      await fetch('/api/reports', { method: 'POST' }); // placeholder — use direct API
      // Record movement via a dedicated call
    ...
    <div className="space-y-5">
      ...
      {tab === 'productos' && (
        <>
          ...
          <div className="card overflow-hidden">
            ...
            : filtered.length === 0 ? <EmptyState icon={Package} title="No hay productos" description="Agrega tu primer producto" action={<button onClick={openNew} className="btn-primary">Agregar</button>}/>
            : (
              <div className="overflow-x-auto">
                <table className="w-full text-sm">
                  <thead><tr className="border-b border-[#21262d]">{['Producto','Categoría','Proveedores','Stock','Precio','Costo','Margen',''].map(h=><th key={h} className="text-left px-4 py-3 text-xs font-medium text-[#8b949e] uppercase tracking-wide">{h}</th>)}</tr></thead>
                  <tbody>
                    {filtered.map(p => {
                        ...
                        <tr key={String(p.id)} className="border-b border-[#21262d] last:border-0 table-row-hover">
                          ...
                          <td className="px-4 py-3">
                            <div className="flex items-center gap-1">
                              ...
                              <button onClick={()=>setDeleteTarget(p)} className="p-1.5 rounded-lg text-[#6e7681] hover:text-red-400 hover:bg-red-500/10 transition-colors"><Trash2 className="w-3.5 h-3.5"/></button>
                            </div>
                          </td>
                        </tr>
                      );
      ...
      <Modal open={showHistoryModal} onClose={()=>setShowHistoryModal(false)} title={`Historial — ${String(historyProduct?.name??'')}`} size="xl">
        <div className="flex justify-end mb-4">
          <button onClick={()=>setShowMoveModal(true)} className="btn-primary flex items-center gap-2"><ArrowRightLeft className="w-4 h-4"/>Registrar movimiento</button>
        </div>
