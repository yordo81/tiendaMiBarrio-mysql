...
export default function GastosPage() {
  ...
  const [form, setForm] = useState({ category_id:'', description:'', amount:0, product_id:'', product_quantity:0, date:'' });

  const load = useCallback(async () => {
    const [e, c, p] = await Promise.all([api.getExpenses(), api.getExpenseCategories(), api.getProducts()]);
    setExpenses(e); setCategories(c); setProducts(p as R[]); setLoading(false);
  }, []);
  useEffect(() => { load(); }, [load]);
  ...
  async function handleSave() {
    ...
    setSaving(true);
    try {
      await api.createExpense({ ...form, category_id: form.category_id || null, product_id: form.product_id || null, product_quantity: form.product_quantity || null, date: form.date || undefined });
      toast.success('Gasto registrado'); setShowModal(false);
      setForm({ category_id:'', description:'', amount:0, product_id:'', product_quantity:0, date:'' }); load();
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="card overflow-hidden">
        ...
        :filtered.length===0?<EmptyState icon={TrendingDown} title="Sin gastos" description="Registra el primer gasto" action={<button onClick={()=>setShowModal(true)} className="btn-primary">Registrar</button>}/>:(
      ...
      <Modal open={showModal} onClose={()=>setShowModal(false)} title="Registrar gasto" size="md">
        <div className="space-y-4">
          ...
          <div className="p-3 bg-[#0d1117] rounded-xl border border-[#21262d] space-y-3">
            ...
            {form.product_id&&<div><label className="label">Cantidad</label><input type="number" min="0.01" step="0.01" className="input" value={form.product_quantity||''} onChange={e=>setForm(f=>({...f,product_quantity:parseFloat(e.target.value)||0}))}/></div>}
          </div>
